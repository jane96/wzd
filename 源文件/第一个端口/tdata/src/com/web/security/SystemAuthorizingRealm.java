...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		User user = null;
		try{
			String userString = des.base64encoder.encode(des.encrypt(token.getUsername().getBytes(), des.PASSWORD));
			user = getSystemService().getUserByName(userString);
			System.out.println("token.getUserName:" + user.getName()+"-----------");
		}catch(Exception e){
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		User user = getSystemService().getUserByName(principal.getLoginId());
		if (user != null) {
	...
	public static class Principal implements Serializable {
		...
		public Principal(User user) {//通过账户名验证
			this.id = user.getName();
			this.loginId = user.getName();
